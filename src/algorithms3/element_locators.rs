...
mod tests {
    ...
    fn finding_element_test() {
        ...
        for elem_index in 0..triangulation.elements().len() {
            for face_index in 0..triangulation.elements()[elem_index].faces_as_points_tuples(&pts).len() {
                ...
                assert_eq!(LocationResult::OnFace(T4Index(elem_index), face_index), locate_element_containing(T4Index(elem_index), triangulation.elements(), triangulation.nodes(), &face_center));
                ...
                for face_index_second in 0..face_index {
                    ...
                    let y = (triangulation.nodes()[(common_nodes.0).0].y + triangulation.nodes()[(common_nodes.1).0].y) / 2.;
                    let z = (triangulation.nodes()[(common_nodes.0).0].z + triangulation.nodes()[(common_nodes.1).0].z) / 2.;
    ...
    fn find_common_nodes(face_1: (N3Index, N3Index, N3Index), face_2: (N3Index, N3Index, N3Index)) -> (N3Index, N3Index) {
