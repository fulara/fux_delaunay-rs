...
pub fn insert_into_element(triangulation: &mut Triangulation2,
                           element_index: T3Index,
                           new_node_index: N2Index)
                           -> (T3Index, T3Index, T3Index) {
    ...
    let left_element = Triangle::new(triangulation.nodes(),
                                     original_elements_nodes[0],
                                     original_elements_nodes[1],
                                     new_node_index);
    let top_element = Triangle::new(triangulation.nodes(),
                                    original_elements_nodes[1],
                                    original_elements_nodes[2],
                                    new_node_index);

    assert_eq!(*left_element.nodes(),
               [original_elements_nodes[0], original_elements_nodes[1], new_node_index]);
    assert_eq!(*top_element.nodes(),
               [original_elements_nodes[1], original_elements_nodes[2], new_node_index]);
...
fn update_neighborhood(triangulation: &mut Triangulation2,
                       for_index: Option<T3Index>,
                       n1: N2Index,
                       n2: N2Index,
                       update_with: T3Index) {
    if let Some(updated_element_index) = for_index {
        let updated_element: &mut Triangle = &mut triangulation.elements_mut()
...
pub fn insert_in_edge(triangulation: &mut Triangulation2,
                      element_index: T3Index,
                      inserted_node_index: N2Index,
                      edge_index: usize)
                      -> (T3Index, T3Index) {
    ...
    let mut new_element = Triangle::new(triangulation.nodes(),
                                        inserted_node_index,
                                        third_node_index,
                                        second_node_index);
...
mod tests {
    ...
    fn insertion_in_edge_test() {
        ...
        assert_eq!(Triangle::new_exact([N2Index(0), N2Index(1), N2Index(2)],
                                       [Some(T3Index(1)), Some(T3Index(2)), None]),
                   triangulation.elements()[0]);
        assert_eq!(Triangle::new_exact([N2Index(0), N2Index(3), N2Index(1)],
                                       [None, None, Some(T3Index(0))]),
                   triangulation.elements()[1]);
        assert_eq!(Triangle::new_exact([N2Index(1), N2Index(4), N2Index(2)],
                                       [None, None, Some(T3Index(0))]),
                   triangulation.elements()[2]);

        insert_in_edge(&mut triangulation, T3Index(0), N2Index(5), 2);
        assert_eq!(4, triangulation.elements().len());
        assert_eq!(Triangle::new_exact([N2Index(5), N2Index(1), N2Index(2)],
                                       [Some(T3Index(3)), Some(T3Index(2)), None]),
                   triangulation.elements()[0]);
        assert_eq!(Triangle::new_exact([N2Index(0), N2Index(3), N2Index(1)],
                                       [None, None, Some(T3Index(3))]),
                   triangulation.elements()[1]);
        assert_eq!(Triangle::new_exact([N2Index(1), N2Index(4), N2Index(2)],
                                       [None, None, Some(T3Index(0))]),
                   triangulation.elements()[2]);
        assert_eq!(Triangle::new_exact([N2Index(5), N2Index(0), N2Index(1)],
                                       [None, Some(T3Index(1)), Some(T3Index(0))]),
