...
#[derive(Debug, PartialEq)]
#[repr(C)]
pub struct CApiTriangulation2 {
    pub elements: *mut CApiElement3,
    pub element_count: usize,
}

fn serialize_and_save_data2(nodes: &Vec<CApiPoint2>) {
...
pub fn deserialize_data2(path_to_data: &str) -> ::std::io::Result<Vec<CApiPoint2>> {
...
#[no_mangle]
pub fn generate_triangulation2(points: *mut CApiPoint2,
                              point_count: libc::int32_t,
                              triangulation_data: *mut CApiTriangulation2) {
    let points: &[CApiPoint2] = unsafe { from_raw_parts(points, point_count as usize) };
    serialize_and_save_data2(&Vec::from(points));
