...
pub fn circumsphere_side(p: &Point3, q: &Point3, r: &Point3, s: &Point3, t: &Point3) -> SphereSide {
    ...
    let det = Matrix4::new(ptx, pty, ptz, pt2,
                           rtx, rty, rtz, rt2,
                           qtx, qty, qtz, qt2,
                           stx, sty, stz, st2).determinant();
...
mod tests {
    ...
    fn quick_check_test(x: f64, y: f64, z: f64, r: f64) {
        ...
        assert_eq! (SphereSide::Inside, circumsphere_side(&a, &b, &c, &d, &Point3::new(x, y, z)));


        let rotations: [Basis3<f64>; 3] = [
            Rotation3::from_angle_x(Rad(-f64::consts::PI / 50.)),
        ...
        for i in 1..10 {
            ...
            for rotation in rotations.iter() {
                ...
                for _ in 0..100 {
                    ...
                    if multiplier < 0.99 {
                        assert_eq!(SphereSide::Inside, side);
                    } else {
                        assert_eq!(SphereSide::Outside, side);
                    }
                }
            }
        }
